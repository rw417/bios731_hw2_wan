...
$$
which does not depend on $\theta$. So while $g(\theta|\theta^{(k)})$ may be greater than the log-likelihood, its difference is constant w.r.t $\theta$. As such, $g(\theta|\theta^{(k)})$ still works as a minorizing function.
...
**E-Step**
The memory-less property of the exponential distribution gives us
...
**M-Step**
The M-step is to maximize the Q-function. Taking the first derivative of the Q-function and setting it to 0 produces
$$\begin{aligned}
